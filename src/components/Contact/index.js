...
const Contact = () => {
    ...
    useEffect(() => {
        if ((Math.abs(contactRef.current.getBoundingClientRect().top)) < (window.screen.height / 2)) {
            dispatch(elementOnScreen('contact'));
        }
    }, [scrolling])

    return (
        <div ref={contactRef} className='displayContainer' id='contact'>
            <div className='startline'>
                <div className='reversedLine'></div>
                <div className='circle'></div>
                <div className='line'></div>
            </div>
            <div className='h-full flex flex-col justify-center ml-[50px]'>
                <form action="https://formsubmit.co/79d18c54d412fbdf60d317e6aafb2958" method="POST" 
                className='text-black h-100% flex flex-col justify-center items-start'>
                    <div className='text-white text-[1.5em]'>Contact Me</div>
                    <input type="text" name="name" placeholder=' Name' className='1em' required />
                    <input type="email" name="email" placeholder=' Email' className='1em' required />
                    <textarea name='body' placeholder=' Message' className='h-[100px] w-full' required/>
                    <button type="submit" className='bg-yellow rounded-xl p-[5px]'>Send</button>
                </form>
                <div className='flex flex-row items-end justify-start mb-[10px]'>
                    <svg height='20px' viewBox="0 0 128 128" className='mr-[10px]'>
                    ...
                    <a href='https://linkedin.com/in/jerrendang' target='_blank' className='hover:opacity-[.7] underline hover:no-underline text-yellow'>LinkedIn</a>
                </div>
                <div className='flex flex-row items-center justify-start mb-[10px]'>
                    ...
                    <a href='https://github.com/jerrendang' target='_blank' className='hover:opacity-[.7] underline hover:no-underline text-yellow'>Github</a>
                </div>
                <div className='flex flex-row items-center justify-start'>
                    <svg height='20px' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className='mr-[10px]'>
                        <path d="M48 64C21.5 64 0 85.5 0 112c0 15.1 7.1 29.3 19.2 38.4L236.8 313.6c11.4 8.5 27 8.5 38.4 0L492.8 150.4c12.1-9.1 19.2-23.3 19.2-38.4c0-26.5-21.5-48-48-48H48zM0 176V384c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V176L294.4 339.2c-22.8 17.1-54 17.1-76.8 0L0 176z" 
                        fill='white'
                    /></svg>
                    <div>djerren@gmail.com</div>
                </div>
            </div>
        </div>
    )
